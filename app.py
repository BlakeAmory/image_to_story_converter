...
# Initialize ChatTTS (do this at the module level to avoid reloading for each request)
chat = ChatTTS.Chat()
...
def generate_story_from_text(scenario: str) -> str:
    ...
    LangChain is also used for chaining purposes
...
def generate_speech_from_text(message: str) -> None:
    """
    A function using ChatTTS to generate speech from text
    :param message: short story generated by the model
    :return: None (saves the audio file)
    """
    wavs = chat.infer([message])
    
    try:
        torchaudio.save("generated_audio.wav", torch.from_numpy(wavs[0]).unsqueeze(0), 24000)
    except:
        torchaudio.save("generated_audio.wav", torch.from_numpy(wavs[0]), 24000)
...
def main() -> None:
    ...
    :return: None
    """
    st.set_page_config(page_title="IMAGE TO STORY CONVERTER", page_icon="🖼️", layout="centered")
